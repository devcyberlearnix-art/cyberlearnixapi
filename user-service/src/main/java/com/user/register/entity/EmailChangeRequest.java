...
/**
 * Tracks the two-step email-change verification flow.
...
@Table(
    ...
    indexes = {
        @Index(name = "idx_ecr_user_id",       columnList = "user_id"),
        @Index(name = "idx_ecr_user_status",   columnList = "user_id, status"),
        @Index(name = "idx_ecr_expiry_status", columnList = "expiry_time, status")
    }
)
@Data
@NoArgsConstructor
...
public class EmailChangeRequest {
    ...
    /** Foreign key to users.id — stored as a plain UUID column for lightweight lookup. */
    ...
    public enum Status {
        ...
        PENDING,
        /** Both emails verified; user.email has been updated. */
        VERIFIED,
